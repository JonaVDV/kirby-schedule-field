...
describe("ScheduleItemArea.vue", () => {
  ...
  const createWrapper = (props = {}) => {
    ...
    return mount(ScheduleItemArea, {
      ...
      stubs: {
        ...
        "k-button": {
          template:
            '<button class="k-button-stub" @click="$emit(\'click\', $event)"><slot /></button>',
          props: ["icon", "variant", "theme"],
        },
        CreateItem: {
          template:
            "<div class=\"create-item-stub\" @click=\"$emit('add-items', {title: 'Test Item', location: 'Test Location', color: '#ff0000'})\"></div>",
  ...
  describe("Navigation Functionality", () => {
    ...
    test("goes to today when today button clicked", async () => {
      ...
      expect(wrapper.vm.currentDay.format("YYYY-MM-DD")).toBe(
  ...
  describe("Unhappy Flows - Error Handling", () => {
    ...
    test("handles moving non-existent event gracefully", () => {
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith(
        "Event not found for instanceId:",
        "non-existent-id",
      );
  ...
  describe("Computed Properties", () => {
    test("calculates startOfWeek correctly", () => {
      ...
      expect(wrapper.vm.startOfWeek.format("YYYY-MM-DD")).toBe(
        testDate.startOf("week").format("YYYY-MM-DD"),
      );
    });
    ...
    test("calculates endOfWeek correctly", () => {
      ...
      expect(wrapper.vm.endOfWeek.format("YYYY-MM-DD")).toBe(
        testDate.endOf("week").format("YYYY-MM-DD"),
      );
    });
    ...
    test("formats week label correctly", () => {
      ...
      expect(wrapper.vm.displayWeeklabel).toBe(
        testDate.endOf("week").format("YYYY-ww"),
      );
    });
  });
